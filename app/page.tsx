...
import { BouncingBall, PawTrail, Reveal } from './reveal';
...
function DashCash() {
  return <div className="product-window dash-mini" aria-label="Prévia da tela de caixa">
    <div className="window-bar"><i/><i/><i/><span>PetFlow · Caixa</span></div>
    <div className="dash-body">
      <div className="dash-total"><small>SALDO EM CAIXA HOJE</small><strong>R$ 2.418</strong><em>Fechamento previsto às 19h</em></div>
...
function DashReports() {
  return <div className="product-window dash-mini" aria-label="Prévia da tela de relatórios">
    ...
    <div className="dash-body">
      ...
      <div className="bars small">{[30,55,40,70,50,65,80].map((h,i)=><i key={i} style={{height:`${h}%`}}/>)}</div>
...
function DashRevenue() {
  return <div className="product-window dash-mini" aria-label="Prévia da tela financeira">
    <div className="window-bar"><i/><i/><i/><span>PetFlow · Financeiro</span></div>
    <div className="dash-body">
      <small>FATURAMENTO DO MÊS</small><strong>R$ 18.240</strong><em>+22% vs. mês anterior</em>
      <div className="mini-spark">{[24,38,30,52,44,68,60,80].map((h,i)=><i key={i} style={{height:`${h}%`}}/>)}</div>
    </div>
  </div>;
}
...
export default function Home() {
  return <main>
    ...
    <PawTrail/>
    <section className="included-wrap shell"><Reveal className="included" id="incluido"><div className="included-intro"><span className="badge">01 / O QUE RESOLVE</span><h2>A resposta para quem<br/>vive <em>apagando incêndio.</em></h2><p>Chega de planilha perdida, caderno de agendamento e estoque no chute. Aqui está tudo que resolve a bagunça do dia a dia — e você só paga pelo que sua loja precisa.</p></div><div className="included-list">{included.map((item,i)=><div key={item}><span>{String(i+1).padStart(2,'0')}</span><p>{item}</p><span className="check-badge"><Check/></span></div>)}</div></Reveal></section>
    <section className="dash-strip shell"><span className="dash-strip-label">TELAS QUE VOCÊ USA TODO DIA</span><div className="dash-grid">{[DashCash,DashReports,DashRevenue].map((C,i)=><Reveal key={i} delay={i*100}><C/></Reveal>)}</div></section>
    <section className="benefits shell" id="solucao"><Reveal className="section-heading"><div><span>02 / NA ROTINA</span><h2>O que muda no<br/><em>dia a dia da loja.</em></h2></div><p>O sistema reduz tarefas repetitivas, evita informações espalhadas e entrega uma visão clara da operação — no balcão ou fora da loja.</p></Reveal><div className="benefit-grid">{benefits.map(({icon:Icon,title,text},i)=><Reveal key={title} delay={i*90}><article><span className="number">0{i+1}</span><Icon/><h3>{title}</h3><p>{text}</p></article></Reveal>)}</div></section>
    <div className="ball-strip shell"><BouncingBall/><p>Simples de aprender, fácil de usar todo santo dia.</p></div>
    <section className="project-section shell" id="projetos"><Reveal className="section-heading"><div><span>03 / PROJETOS REAIS</span><h2>Não é só ideia.<br/><em>Já está funcionando.</em></h2></div><p>Produtos desenvolvidos para o mercado pet, com experiência real em reservas, serviços e gestão operacional.</p></Reveal>
